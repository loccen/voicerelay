...
- 发送成功后自动清空手机端输入框
- 手机端保留发送历史，点击历史项可回填输入框，也可以一键清空历史
- 手机端和菜单栏 App 都会显示本次输入统计、历史累计统计，并估算节省时间
...
- 历史累计：累计发送字数、累计真实输入耗时和估算节省时间

节省时间默认按 `60 字/分钟` 的普通中文手打速度估算，并扣除手机端本次实际输入耗时。你可以在菜单栏 App 里修改自己的打字速度，也可以打开菜单里的“打字速度测试”去 `https://dazi.kukuw.com/` 测试后回填。

历史和统计会同时保存在手机浏览器本地和 Mac 本地服务的 `.voicerelay-data.json` 中，所以清除手机缓存后仍可从服务端恢复。发送历史可以在手机端或菜单栏 App 里清空；清空历史不会清空累计统计。测试期间产生的累计统计也可以在手机端或菜单栏 App 里重置。
